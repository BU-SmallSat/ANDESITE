...
{
	I2c.begin();	// Initialize I2C library
	I2c.timeOut(5000);
	//INCLUDE A BUS SCAN FOR AVAILABLE DEVICES. if none available then restart?
}

void LSM9DS0::I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
	// Begin transmission at device write address
	if(I2c.write(address, subAddress, data)){
		Serial.println("unsuccessful write! Timeout occured");
		//inculde error handling code for unsuccessful writes. restart?
	}
}
...
uint8_t LSM9DS0::I2CreadByte(uint8_t address, uint8_t subAddress)
{
	...
	// read one byte from the subAddress register on the address device
	if(I2c.read(address, subAddress, (uint8_t) 1)){
		Serial.println("Unsuccessful read! Timeout occured - check connection to device.");
		//include error handling code for unsuccessful reads. restart?		
	} 
	
	while (I2c.available() < 1){} // Wait until data becomes available
	data = I2c.receive(); // Read register data into `data` variable
...
{  
	...
	if(I2c.read(address, subAddress, count, dest)){
		Serial.println("Unsuccessful read! Timeout occured - check connection to device.");
		//include error handling code for unsuccessful reads. restart?
	}
}