...
// Start Science mode
int AndesiteWSN::scienceMode(SPISettings ADC_Settings) {
    /******************************************
    *****IMPORTANT:: TURN OFF RADIOS HERE******
    ******************************************/
    _ADC.initialize(ADC_Settings);
        ...
        */
        switch(_science_mode_state){
            case 1:
                ...
                AndesiteCollect::mag(ADC_Settings);
                ++count_mag; 
                _science_mode_state = 0;
                break;
...
void AndesiteWSN::listenMuleMessage() {
    /*****************************************
    *****IMPORTANT:: TURN ON RADIOS HERE******
    *****************************************/
    unsigned long timer_start = millis();
    while ( _Orbit._latitude < _Orbit._lat_mode_switch ){
        if (_Radio.listen() == ACDH_MULE_ADDR) {
            Serial.println(":: here listen?");
            //Serial.println("first letter in message from mule");
            //Serial.println(_Radio._message[0]);
            /*
            ...
            */
            ...
            if ( !_Orbit.setLatitude() ) { Serial.println("GPS Fail."); }
        }
    }
}
