...
void loop()
{
  char command[] = {'M', 'Y'};
  byte atPacket[8];
  ATLocalRequest().atLocalRequest(atPacket, dataId, command);
  
  char parameters[] = {0, 1, 2};
  byte atParameterPacket[8 + sizeof(parameters)];
  ATLocalRequest().atLocalRequest(atParameterPacket, dataId, command, parameters, sizeof(parameters));
  ...
  printPacket(atPacket, sizeof(atPacket));
  printPacket(atParameterPacket, sizeof(atParameterPacket));
  delay(1000);
}
