...
void loop()
{
  //If the clock is now high from a previously low state
  //increment the clock counter and mark it as high.
  if(clock_level == 0 && digitalRead(4) == HIGH){
    clock_level = 1;
    clock_count++;
  } else if(clock_level == 1 && digitalRead(4)==LOW){
    //If the clock is now in a low state from a high one,
    //mark the level as low.
    clock_level =0;
  } else{
