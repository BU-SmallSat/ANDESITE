...
class ADCThread(WorkerThread):
    ...
    def processResponse(self, string):
        ...
        #todo: TEST THE BAR MAGNET COMMAND BELOW
        elif "AG:BarMagnet" in string:
            axis = string[12]
            strength = int(string[14:19])#let's be a little paranoic and make strength a 5 digit integer
            time_seconds = int(string[20:])
            self.MTQ.BarMagnet(axis, strength, time_seconds)
