...
MuleCalibrationRadio::MuleCalibrationRadio() {
    _cmd_start      = 0x2;
    _cmd_done       = 0x1;
...
int MuleCalibrationRadio::send(uint8_t *message, uint8_t len, uint8_t addr) {
...
// Send message to the given radio address
int MuleCalibrationRadio::sendCommand(uint8_t message[], uint8_t len, uint8_t addr) {
    digitalWrite(RF_CS_PIN, LOW);
    digitalWrite(ADS_CS_PIN, HIGH);
    digitalWrite(SD_CS_PIN, HIGH);
...
int MuleCalibrationRadio::listen() {
    digitalWrite(RF_CS_PIN, LOW);
    digitalWrite(ADS_CS_PIN, HIGH);
    digitalWrite(SD_CS_PIN, HIGH);
...
// Return command
uint8_t MuleCalibrationRadio::getCommand(uint8_t num) {
    if ( num == 0x2 )
        return _cmd_start;
    if ( num == 0x1 )
        return _cmd_done;
