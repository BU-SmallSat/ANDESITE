...
class EPSThread(WorkerThread):
    ...
    def resetWatchdog(self,period):
        self.EPS.set_comms_watchdog_period(period)
        time.sleep(self.waitTime)
        newPeriod = self.EPS.get_comms_watchdog_period()
        if newPeriod is not period:
            self.executiveQueue.put("EP:WatchdogResetFailure")
        else:
            self.executiveQueue.put("EP:WatchdogResetSuccess")
    ...
    def processResponse(self, string):
        if string == "PC:restartEPS":
            self.restartEPS()
        elif string == "PE:WatchdogReset90":
            self.resetWatchdog(90)
