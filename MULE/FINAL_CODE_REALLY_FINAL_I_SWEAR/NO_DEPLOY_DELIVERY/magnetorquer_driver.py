...
class magnetorquer_driver :
    ...
    def start_dipole(self, x_axis, y_axis, z_axis, duration):
        ...
        x1 = x_axis % 256  # LEAST SIGNIFICANT BYTE
        x0 = x_axis // 256  # MOST SIGNIFICANT BYTE

        y1 = y_axis % 256  # LEAST SIGNIFICANT BYTE
        y0 = y_axis // 256  # MOST SIGNIFICANT BYTE

        z1 = z_axis % 256  # LEAST SIGNIFICANT BYTE
        z0 = z_axis // 256  # MOST SIGNIFICANT BYTE

        dur1 = duration % 256  # LEAST SIGNIFICANT BYTE
        dur0 = duration // 256  # MOST SIGNIFICANT BYTE

        reply = self.send_command([2, 0x06, x1, x0, y1, y0, z1, z0, dur1, dur0])
    ...
    def setup_watchdog(self,period):
        self.set_parameter(WATCHDOG_ID, period,1)
        self.get_parameter(WATCHDOG_ID,1)
    ...
    def timeBDOT(self,mins):
        ...
        ### HEX FUNCTION CONVERTS AN INTEGER INTO A STRING BEGINING WITH '0x',
        ### SO THE THEE LINES OF CODE BELOW WERE PROBABLY WRONG.
        ### IT IS EOGUTH TO JUST PUT THE INTEGERS INTO THE BYTELIST
        # high_hex = hex(high)
        # low_hex = hex(low)
        # self.start_bdot([high_hex,low_hex])
        ### WHAT SHOULD BE DONE, IS BELOW:
        self.start_bdot([hex, hex])

    def BarMagnet(self, axis, strength, time_seconds):
        # time in seconds
        duration_ms = time_seconds*1000 # start_dipole function need milliseconds for input
        if axis == 'x':
            self.start_dipole(strength, 0, 0, duration_ms)
        elif axis == 'y':
            self.start_dipole(0, strength, 0, duration_ms)
            # self.start_dipole([0x00, 0x00], [0x00, 0x00], [0x00, 0x00], [0x27, 0x10])
        elif axis == 'z':
            self.start_dipole(0, 0, strength, duration_ms)
            # self.start_dipole([0x00, 0x00], [0x00, 0x00], [0x00, 0x00], [0x27, 0x10])
