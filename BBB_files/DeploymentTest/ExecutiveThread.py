...
Should it wait till after low power mode to sift through these commands?
    Should it execute all commands that it recieved during low power mode?
...
Edit the dictionaries to do error handling if the string isnt correct
'''
...
# IMPROTANT:: need a method to start the radio on the Mule if the node ejected was the first one
startNetwork = 0
