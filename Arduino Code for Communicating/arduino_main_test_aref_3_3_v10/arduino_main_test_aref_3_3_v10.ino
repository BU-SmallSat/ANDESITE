...
const int MODE_IDLE = 0xa;
const int MODE_STARTUP = 0xb;
const int MODE_SCIENCE = 0xb;
const int MODE_TRANSMISSION 0xc;
...
void setup()
{
  ...
  //Sends initial notification packet
  byte notificationPacket[20];
  TX().txCommandPacket(notificationPacket, coord.getSH(), 
                coord.getSL(), coord.getMY(), 0, 0, 0, 0, 0);
  sendPacket(notificationPacket, sizeof(notificationPacket));
}
...
void loop()
{
  ...
  if(out.peek() == 0x7E) {
    ...
    if(RX.()getDataType(received) == MODE_IDLE) {
      mode = 0;
    } else if(RX.()getDataType(received) == MODE_STARTUP) {
      mode = 1;
    } else if(RX.()getDataType(received) == MODE_SCIENCE) {
      mode = 2;
    } else if(RX.()getDataType(received) == MODE_TRANSMISSION) {
      mode = 3;
    }
  }
