...
int led = 5; //pin 30 is the tx led
...
void setup()
{
  ...
  pinMode(53,OUTPUT); //RADIO CS
  pinMode(chipSelect,OUTPUT); //SD CS
...
//  unsigned long T_int = READ_TEMP(OFCT_int);      //page 35 188mv @ 25*C & 405uV/*C
  ...
  float voltageX = ((5.0/8388608.0)*X_raw) ; 
  float voltageY = ((5.0/8388608.0)*Y_raw);
  float voltageZ = ((5.0/8388608.0)*Z_raw);

  
  String data = String(iteration)+", " + millis();
  data += ", " + String(voltageX,10) +", " + String(voltageY,10) + ", " + String(voltageZ,10);;
...
void setupADC(){
  ...
  SPI.transfer(0x00);
  SPI.transfer(SCR0_PGA128_SPS320);  
  delayMicroseconds(10000);
