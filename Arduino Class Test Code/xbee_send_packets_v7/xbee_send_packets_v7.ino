...
void loop()
{
  if(!begin) {
    ...
    for(int i = 0; i < out.available() && counter < startLength; i++)
    { 
      //if(counter < startLength)
      { 
        received[counter] = out.read();
        Serial.print(received[counter], HEX);
        Serial.println(" " + String(i) + " " + String(counter));
        counter++;
      }
    }
  }
    if(received[personalIdIndex] == startCom) {
      begin = true;
    }
  //}
  ...
  if(begin) {
    ...
    for(int x = 0; x < packetToSendLength; x++) {
      ...
      //Serial.println(packetToSend[x], HEX);
      /*for (int i = 0; i < 26; i++) {
